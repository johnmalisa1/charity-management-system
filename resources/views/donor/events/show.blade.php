...
<div class="container py-8">
    ...
    <!-- ✅ Back button -->
    <div class="mb-3">
        <a href="{{ route('donor.events') }}" class="btn btn-secondary">
            ← Back to Events
        </a>
    </div>
    ...
    <div class="card bg-dark text-light mb-4">
        <div class="card-body">
            ...
            <p class="card-text">
                Date: {{ $event->start_time->format('M d, Y') }} <br>
                Location: {{ $event->location }}
            </p>

            <!-- ✅ Join/Cancel buttons using donors pivot -->
            @if($event->donors->contains(Auth::id()))
                <form action="{{ route('donor.events.cancel', $event->id) }}" method="POST" class="d-inline">
                    @csrf
                    <button type="submit" class="btn btn-danger">Cancel Event</button>
                </form>
            @else
                <form action="{{ route('donor.events.join', $event->id) }}" method="POST" class="d-inline">
                    @csrf
